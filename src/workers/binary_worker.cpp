...
using ProtoDumper = QByteArray(*)(QVariant const& msg);
using FramesDumper = QByteArray(*)(QVariant const& msg, ProtoDumper intoProto, BinaryConfig const& config);
...
static QByteArray dumpMsgpackProto(QVariant const& msg) {
...
static uint16_t CRC16_Modbus (const char *nData, size_t wLength)
{
    static const unsigned wCRCTable[] = {
    ...
    while (wLength--)
    {
        nTemp = unsigned(*nData++) ^ wCRCWord;
        wCRCWord >>= 8;
        wCRCWord ^= wCRCTable[nTemp];
    }
    return uint16_t(wCRCWord);

}

static QByteArray dumpSlipFrames(QVariant const& msg, ProtoDumper intoProto, BinaryConfig const& config) {

    QByteArray buffer;
    auto wrapped = intoProto(msg);
    if (auto crc = config.crc) {
        switch (*crc) {
        case BinaryCrc::modbus: {
            auto c = CRC16_Modbus(wrapped.constData(), unsigned(wrapped.size()));
            wrapped.append(reinterpret_cast<const char*>(&c), sizeof(c));
            break;
        }
        }
    }
    slipa::Write({wrapped.data(), size_t(wrapped.size())}, [&](string_view part) {
        buffer.append(part.data(), int(part.size()));
	});
	buffer += slipa::END;
	return buffer;
}
...
{
    auto frame = d->framesDumper(msg, d->protoDumper, d->config);
    SendBinary({frame.constData(), size_t(frame.size())});
}
